...
describe("Heroku Commands", () => {
  ...
  describe("heroku list apps <app name>", () => {
    ...
    describe("when given an argument <app name>", done => {
      it("returns a list of the apps filtered by <app name>", done => {
        room.user.say("Damon", "hubot heroku list apps staging").then(() => {
    ...
    describe("when the command is called without arguments" , done => {
      it("returns a list of all apps", done => {
        room.user.say("Damon", "hubot heroku list apps").then(() => {
  ...
  describe("heroku info <app>", () => {
    it("gets information about the app's dynos", done => {
      ...
      room.user.say("Damon", "hubot heroku info shield-global-watch").then(() => {
  ...
  describe("heroku dynos <app>", () => {
    it("lists all dynos and their status", done => {
      ...
      room.user.say("Damon", "hubot heroku dynos shield-global-watch").then(() => {
  ...
  describe("heroku rollback <app> <version>", () => {
    ...
    it("tells the user about a bad supplied version", done => {
      room.user.say("Damon", "hubot heroku rollback shield-global-watch v999").then(() => {
  ...
  describe("heroku restart <app> <dyno>", () => {
    it("restarts the app", done => {
      ...
      room.user.say("Damon", "hubot heroku restart shield-global-watch").then(() => {
    ...
    it("restarts all dynos of a process", done => {
      ...
      room.user.say("Damon", "hubot heroku restart shield-global-watch web").then(() => {
    ...
    return it("restarts specific dynos", done => {
      mockHeroku
        .delete("/apps/shield-global-watch/dynos/web.1")
        .reply(200, {});

      room.user.say("Damon", "hubot heroku restart shield-global-watch web.1").then(() => {
  ...
  describe("heroku config <app>", () => {
    it("gets a list of config keys without values", done => {
      ...
      room.user.say("Damon", "hubot heroku config shield-global-watch").then(() => {
  ...
  describe("heroku config:set <app> <KEY=value>", () => {
    ...
    it("sets config <KEY=value>", done => {
      ...
      room.user.say("Damon", "hubot heroku config:set shield-global-watch CLOAK_ID=example.com").then(() => {
    ...
    it("handles URLs", done => {
      ...
      room.user.say("Damon", "hubot heroku config:set shield-global-watch PUSHER_URL=http://c0d9g8najfcc4634kd3:cf2eeas0d8dghfa847d@api.pusherapp.com/apps/1234").then(() => {
    ...
    it("handles comma delimited strings", done => {
      ...
      room.user.say("Damon", "hubot heroku config:set shield-global-watch COMMA_DELIMITED_STRING=MiD,DA,MDe").then(() => {
    ...
    it("handles text strings", done => {
      ...
      room.user.say("Damon", "hubot heroku config:set shield-global-watch SENTENCE=\"Don't stop believin.\"").then(() => {
    ...
    return it("handles RSA secret keys", done => {
  ...
  describe("heroku config:unset <KEY>", () =>
    it("unsets config <KEY>", done => {
      ...
      room.user.say("Damon", "hubot heroku config:unset shield-global-watch CLOAK_ID").then(() => {
  ...
  describe("heroku ps:scale", () => {
    ...
    it("scales dynos", done => {
      mockRequest({ quantity: "2" });

      room.user.say("Damon", "hubot heroku ps:scale shield-global-watch web=2").then(() => {
